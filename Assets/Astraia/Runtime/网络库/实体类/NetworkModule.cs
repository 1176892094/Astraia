...
// # Author: 云谷千羽
// # Version: 1.0.0
// # History: 2026-09-02 21:09:38
// # Recently: 2026-09-02 21:32:38
// # Copyright: 2024, 云谷千羽
...
namespace Astraia.Net
{
    public abstract class NetworkModule : Export
    {
        ...
        protected void SendTargetRpcInternal(NetworkClient client, string name, int hash, MemoryWriter writer, int pass)
        {
            if (!NetworkManager.isServer)
            {
                Debug.LogError("调用 {0} 但是服务器不是活跃的。对象名称：{1}".Format(name, owner.name), owner);
        ...
        private static bool SyncVarEqualNetworkModule<T>(T entity, NetworkVariable variable) where T : NetworkModule
        {
            ...
            return variable.Equals(new NetworkVariable(newValue, newIndex));
        }
