...
// # Author: 云谷千羽
// # Version: 1.0.0
// # History: 2026-09-02 17:09:50
// # Recently: 2026-09-02 17:22:50
// # Copyright: 2024, 云谷千羽
...
namespace Astraia
{
    public static partial class AssetManager
    {
        ...
        {
            if (GlobalSetting.Instance.UseSimulate)
            {
            ...
            verified = SelectManifest(streamResult, clientResult, serverResult);
            ...
            if (!verified.IsRemote && clientResult.Version >= verified.Version)
            {
            ...
            foreach (var (name, bundle) in verified.Bundles)
            {
            ...
            var success = await DownloadBundlesAsync(toDownload, verified.IsRemote);
            if (success)
            {
                await SaveManifestToPersistentAsync(verified);
        ...
        private static async Task SaveManifestToPersistentAsync(Verified verified)
        {
            var json = JsonManager.ToJson(new Package(verified.Version, verified.Bundles.Values.ToList()));
        ...
        private static async Task<Verified> LoadManifestFromStreamingAsync()
        {
        ...
        private static async Task<Verified> LoadManifestFromPersistentAsync()
        {
            var path = GlobalSetting.PersistentPath.Format(GlobalSetting.VERIFY);
        ...
        private static async Task<Verified> LoadManifestFromServerAsync()
        {
        ...
        private static Verified LoadManifestFromJson(string json, bool remote)
        {
            var bundles = new Dictionary<string, Bundle>();
            if (string.IsNullOrEmpty(json))
            {
                return new Verified(-1, bundles, false, remote);
            }
            ...
            return new Verified(result.Version, bundles, true, remote);
        }

        private static Verified SelectManifest(Verified stream, Verified client, Verified server)
        {
            if (server.IsLoaded)
            {
    ...
    internal readonly struct Verified
    {
        ...
        public Verified(int version, Dictionary<string, Bundle> bundles, bool isLoaded, bool isRemote)
        {
            Version = version;
            Bundles = bundles;
            IsLoaded = isLoaded;
