    ...
    internal static partial class FormManager
    {
        public static async Task<bool> WriteScripts(string reason)
        {
            try
            {
                ...
                Loaded = true;
                var instance = false;
                formItem.Add(GlobalSetting.ASMDEF, GlobalSetting.LoadText(TextAsset.程序集).Replace("REPLACE", GlobalSetting.DEFINE));
        ...
        private static (string, string) WriteTable(string className, Dictionary<string, string> fields)
        {
            ...
            var scriptText = GlobalSetting.LoadText(TextAsset.数据表).Replace("Template", className);
            var count = 0;
        ...
        {
            var builder = HeapManager.Dequeue<StringBuilder>();
            var scriptText = GlobalSetting.LoadText(TextAsset.结构体).Replace("Template", className);
        ...
        {
            var builder = HeapManager.Dequeue<StringBuilder>();
            var scriptText = GlobalSetting.LoadText(TextAsset.枚举类).Replace("Template", className);
