...
namespace Astraia
{
    ...
    internal static partial class Hierarchy
    {
...
#endif
        {
            ...
            void DrawWindow()
            {
                var window = EditorEmit.Hierarchy.GetValue<EditorWindow>("s_LastInteractedHierarchy");
                if (window != null)
                {
        ...
        private static void DrawTransform(Rect rect, GameObject target)
        {
            ...
            Texture DrawIcon()
            {
                if (!target)
                {
                    return EditorEmit.sceneIcon.image;
                }
        ...
        private static void DrawComponent(Rect rect, GameObject target)
        {
            ...
            var nameSize = rect.x + EditorEmit.CalcSize(target.name) + 15;
